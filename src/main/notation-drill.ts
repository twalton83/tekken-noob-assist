...
export class NotationDrill extends EventEmitter {
  ...
  onPress(limbs: Limb[], dir: Dir) {
    if (!this.active || this.waitTimer) return; // ignore inputs during the reveal pause
    ...
    if (!ok) {
      ...
      this.pause(REVEAL_MS.wrong, () => { this.idx = 0; this.feedback = null; });
      return;
    }
  ...
  private pause(ms: number, then: () => void) {
